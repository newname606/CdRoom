<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:68:"E:\CdRoom\generate\public/../application/fish\view\dongtai\save.html";i:1610347065;}*/ ?>
<!--_meta 作为公共模版分离出去-->
...
<form action="" method="post" class="form form-horizontal" id="form-member-add" enctype="multipart/form-data">
    <div class="row cl">
        <label class="form-label col-xs-4 col-sm-3">动态详情内容</label>
        <div class="formControls col-xs-8 col-sm-9">
            <texteara class="text"  style="width:800px; min-height:600px;" name="text" id="text"><?php echo !empty($row['text'])?$row['text']:''; ?></texteara>
        </div>
    </div>
    <div class="row cl">
        <label class="form-label col-xs-4 col-sm-3">发布者名称</label>
        <div class="formControls col-xs-8 col-sm-9">
            <input type="text" class="input-text" value="<?php echo !empty($row['uname'])?$row['uname']:''; ?>" name="uname">
        </div>
    </div>
    <div class="row cl">
    ...
    <div class="row cl">
        ...
        <div class="formControls col-xs-8 col-sm-9">
            <select class="btn btn-primary radius" name="labelid" id="labelid">
                <option value="">--请选择--</option>
                <?php if(is_array($label) || $label instanceof \think\Collection || $label instanceof \think\Paginator): $i = 0; $__LIST__ = $label;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$build): $mod = ($i % 2 );++$i;?>
                <option value="<?php echo $build['id']; ?>"
                <?php if(isset($row)){ if($build['id']==$row['roomid']){ ?> selected="selected" <?php }} ?> ><?php echo $build['name']; ?></option>
                <?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
        </div>
    </div>
    ...
    <div class="row cl">
        ...
        <div class="formControls col-xs-8 col-sm-9">
            <input type="text" class="input-text" value="<?php echo !empty($row['title'])?$row['title']:''; ?>" name="title">
        </div>
    </div>
    <div class="row cl">
    ...
    <div class="row cl">
        ...
        <div class="formControls col-xs-8 col-sm-9">
            <input type="text" class="input-text" value="<?php echo !empty($row['content'])?$row['content']:''; ?>" name="content">
        </div>
    </div>

    <input type="hidden" id="textarea" class="textarea" name="textarea">
...
<script type="text/javascript">
    ...
    CKEDITOR.replace('text', { //这里的 mycontent就是上面我们设置的textarea或者input元素的id
            filebrowserUploadUrl: '/fish.php/dongtai/uploadimage?command=QuickUpload&type=Files&responseType=json',
